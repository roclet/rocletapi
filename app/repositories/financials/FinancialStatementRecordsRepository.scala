...
/**
  ...
  */
class FinancialStatementRecordsRepository extends CassandraTable[FinancialStatementRecordsRepository,FinancialStatementRecords]{
  ...
  override def fromRow(r:Row):FinancialStatementRecords={
...
object FinancialStatementRecordsRepository extends FinancialStatementRecordsRepository with RootConnector{
  override lazy val tableName = "fstatements"
  ...
  def getStatements(orgCode:String):Future[Seq[FinancialStatementRecords]]={
    select
      .where(_.orgCode eqs orgCode)
      .fetchEnumerator run Iteratee
      .collect()
  }
  def getStatementsByType(orgCode:String, statementType:String):Future[Seq[FinancialStatementRecords]]={
    select
      .where(_.orgCode eqs orgCode)
  ...
  def getStatementsByCategory(orgCode:String, statementType:String, category:String ):Future[Seq[FinancialStatementRecords]]={
    select
      .where(_.orgCode eqs orgCode)
      .and(_.statementType eqs statementType)
  ...
  def getStatementsByYear(orgCode:String, statementType:String, category:String, year:Int ):Future[Seq[FinancialStatementRecords]]={
    select
      .where(_.orgCode eqs orgCode)
      .and(_.statementType eqs statementType)
      .and(_.category eqs category)
  ...
  def getStatementsByMonth(orgCode:String, statementType:String, category:String, year:Int, month:Int):Future[Seq[FinancialStatementRecords]]={
    select
      .where(_.orgCode eqs orgCode)
      .and(_.statementType eqs statementType)
      .and(_.category eqs category)
...
//  def truncateFinancialStatementRecords(): Future[ResultSet] = {
//    truncate().future()
//  }
}
