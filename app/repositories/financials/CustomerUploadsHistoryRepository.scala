...
import domain.financials.{CustomerUploads, CustomerUploadsHistory}
...
/**
  ...
  */
class CustomerUploadsHistoryRepository extends CassandraTable[CustomerUploadsHistoryRepository, CustomerUploadsHistory] {
  ...
  object reference extends StringColumn(this) with PrimaryKey[String]
  ...
  object txnType extends StringColumn(this)

  // Transaction Type

  override def fromRow(r: Row): CustomerUploadsHistory = {
    CustomerUploadsHistory(
      orgCode(r),
      reference(r),
      date(r),
      accountingCode(r),
...
object CustomerUploadsHistoryRepository extends CustomerUploadsHistoryRepository with RootConnector {
  override lazy val tableName = "custuploadshistory"

  override implicit def space: KeySpace = DataConnection.keySpace

  override implicit def session: Session = DataConnection.session

  def save(customeruploads: CustomerUploadsHistory): Future[ResultSet] = {
    insert
  ...
  def getUploadByOrganisation(orgCode: String): Future[Seq[CustomerUploadsHistory]] = {
    select
      .where(_.orgCode eqs orgCode)
      .fetchEnumerator() run Iteratee.collect()
  }

  def getUploadsYear(orgCode: String, year: Int): Future[Seq[CustomerUploadsHistory]] = {
    select
      .where(_.orgCode eqs orgCode)
      .and(_.year eqs year)
      .fetchEnumerator() run Iteratee.collect()
  ...
  def getUploadsByMonth(orgCode: String, year: Int, month: Int): Future[Seq[CustomerUploadsHistory]] = {
    select
      .where(_.orgCode eqs orgCode)
      .and(_.year eqs year)
  ...
  : Future[Option[CustomerUploadsHistory]] = {
    select
      .where(_.orgCode eqs orgCode)
      .and(_.year eqs year)
      .and(_.month eqs month)
