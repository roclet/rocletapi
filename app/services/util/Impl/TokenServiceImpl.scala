...
class TokenServiceImpl extends TokenService with Service {
  ...
  override def createNewToken(credential: Credential): Future[UserGeneratedToken] = {
    val useraccounts = UserService.apply().getUserByEmail(credential.orgCode, credential.email)
    val results = useraccounts map ((account: Option[User]) => account match {
      case Some(user) =>
  ...
  override def isTokenValid(token: String, password: String): Future[Boolean] = {
    ...
    if (MarginKeys.MARGINTOKENKEY.equalsIgnoreCase(token)) {
      Future {
        true
      }
    } else {
  ...
  def generateToken(user: User, credential: Credential): Future[String] = {
    ...
    val userRole = UserService.apply.getUserRoles(user.orgCode, user.email) map (role => {
      role map (e => e.roleId)
    })
